...
export function RenataMeshyPresence({ messages }: { messages: DbMessage[] }) {
  ...
  const speakSelected = () => {
    ...
    const plain = textForSpeech(msg.content);
    if (!plain) return;
  ...
  async function startMeshy() {
    ...
    try {
      ...
      if (data.taskId) {
        ...
        sessionStorage.setItem(STORAGE_KEY, data.taskId);
      }
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-[#161b22] p-4">
      <div className="mb-3 flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-neutral-500">
            Meshy (image → GLB) + voz del navegador. Solo admins.
          </p>
        </div>
        <Button
          type="button"
      ...
      <p className="mb-3 text-xs text-neutral-400">{meshyLabel}</p>
      ...
      <div
        ...
        style={{
          transform: glbUrl ? `scale(${1 + pulse * 0.06})` : undefined,
      ...
      >
        {glbUrl ? (
          <model-viewer
            ...
            className="h-[min(52vh,420px)] w-full bg-gradient-to-b from-neutral-900 to-black"
          />
        ) : (
          <div className="flex h-[min(52vh,420px)] flex-col items-center justify-center gap-2 px-4 text-center">
